...
// Verze: 1.0.5 (oprava dynamického načítání GTM)
(function(window, document) {
    ...
    const CONFIG = {
        ...
        localStorageKey: 'consentBooster_userConsent_v1.0.5', // Inkrementace verze klíče
    ...
    class ConsentBooster {
        constructor(config) {
            ...
            this._log('ConsentBooster initialized with config (v1.0.5):', this.config);
        ...
        initDefaultConsent() {
            // Inicializace dataLayer a gtag je nutná před jakýmkoliv gtag voláním
        ...
        _showElement(element) {
            if (element) element.style.display = 'flex'; // 'flex' pro modal, 'block' pro banner by mohl být lepší, ale záleží na CSS
        }
        ...
        // Metoda pro načítání GTM, která replikuje standardní snippet
        _loadGtm() {
            if (!this.config.gtmId) return;
            this._log('Preparing to load GTM script for ID:', this.config.gtmId);

            // Ujistíme se, že dataLayer je pole (to už dělá initDefaultConsent, ale pro jistotu)
            window.dataLayer = window.dataLayer || [];

            // Pušnutí 'gtm.start' a 'gtm.js' události PŘED načtením gtm.js souboru
            ...
            gtmScript.onload = () => {
                this._log('GTM script (gtm.js) loaded successfully.');
            };
            gtmScript.onerror = () => {
                this._log('Error loading GTM script (gtm.js).', 'error');
            };
        ...
        // Metoda pro načítání gtag.js (pro přímé GA4)
        _loadGa4Direct() {
            ...
            ga4Script.onload = () => {
                ...
                // gtag('js', new Date()) je voláno automaticky gtag.js knihovnou,
                // ale pro jistotu a konzistenci s dokumentací Google to zde můžeme nechat,
                // nebo se spolehnout, že gtag.js to udělá.
                // Pro gtag.js je klíčové, že funkce gtag je definována před jeho načtením (což initDefaultConsent zajistí).
                window.dataLayer.push({'event': 'gtag.js_loaded'}); // Indikace pro debug
                gtag('js', new Date()); // Standardní inicializace po načtení gtag.js
                gtag('config', this.config.ga4Id);
                this._log('GA4 configured with ID:', this.config.ga4Id);
            };
            ga4Script.onerror = () => {
                this._log('Error loading GA4 script (gtag.js).', 'error');
            };
        ...
        init() {
            ...
            this.initDefaultConsent(); // Zajistí existenci dataLayer a gtag funkce
            this._queryElements();
            ...
            if (this.defaultStateWasUpdatedFromStorage) {
                ...
                if (storedConsent) {
                    ...
                    if (this.elements.banner) {
                        // U banneru je lepší použít 'block' pokud není flexbox nutný pro jeho vnitřní layout
                        this.elements.banner.style.display = this.elements.banner.tagName === 'DIV' ? 'block' : 'flex'; 
                    }
                }
            }

            // Načtení Google skriptů se volá až po zpracování souhlasu
