...
// Verze: 1.0.6 (řešení pro aktivaci GA4 po udělení souhlasu v téže session)
(function(window, document) {
    ...
    const CONFIG = {
        ...
        localStorageKey: 'consentBooster_userConsent_v1.0.6',
    ...
    class ConsentBooster {
        constructor(config) {
            ...
            this.initialConsentDecisionMadeThisSession = false; // Flag pro sledování prvního rozhodnutí
        ...
        _getInitialConsentStateForDefault() {
            ...
            if (this.config.updateDefaultStateFromStorage) {
                ...
                if (storedConsent) {
                    ...
                    this.initialConsentDecisionMadeThisSession = true; // Pokud je v LS, rozhodnutí už padlo
        ...
        updateConsent(newGoogleConsentStateFromUI, isFirstDecisionThisSession = false) {
            ...
            if (isFirstDecisionThisSession) {
                this.initialConsentDecisionMadeThisSession = true;
                // Pokud je to první udělení souhlasu v této session a analytika je povolena
                if (fullConsentState.analytics_storage === 'granted') {
                    this._log('Pushing consent_granted_first_time event to dataLayer.');
                    window.dataLayer.push({'event': 'consent_granted_first_time', 'granted_consent_state': fullConsentState });

                    // Pro přímé GA4 (bez GTM) explicitně znovu nakonfigurujeme
                    if (this.config.ga4Id && !this.config.gtmId) {
                        this._log('Re-triggering GA4 config for ID:', this.config.ga4Id, 'after first consent grant.');
                        gtag('config', this.config.ga4Id); // Mělo by odeslat page_view
                    }
                }
            }
        ...
        // ... (_loadGtm, _loadGa4Direct, loadGoogleScripts beze změny z verze 1.0.5) ...
        // ... (_activateScriptsForGoogleParam beze změny) ...

        _handleAcceptAll() {
            const consentState = {};
            this.config.managedConsentParams.forEach(param => consentState[param] = 'granted');
            // Předáme informaci, zda je to první rozhodnutí v session
            this.updateConsent(consentState, !this.initialConsentDecisionMadeThisSession);
        ...
        _handleSaveSettings() {
            ...
            this.updateConsent(newGoogleConsentState, !this.initialConsentDecisionMadeThisSession);
            this._hideElement(this.elements.settingsModal);
            this._hideElement(this.elements.banner);
        }

        // ... (_handleCloseModal beze změny) ...
        // ... (init beze změny z verze 1.0.5) ...
        // ... (_log beze změny) ...

        // ===================================================================================
        // Zde jsou metody, které zůstaly stejné jako ve verzi 1.0.5 pro kompletnost
        // ===================================================================================
        ...
        _showElement(element) {
            if (element) {
                // Zde můžete upravit, jak se prvky zobrazují, např. přidáním třídy
                // Pro jednoduchost nechávám inline styl, ale třídy jsou lepší pro Webflow
                if (element.hasAttribute('data-cb-modal')) {
                    element.style.display = 'flex'; // Modální okna často používají flex pro centrování
                } else {
                    element.style.display = 'block'; // Banner typicky jako blok
                }
            }
        }
        ...
        // Načítání přímého GA4 (z verze 1.0.5)
        _loadGa4Direct() {
        ...
        init() {
            ...
            if (this.defaultStateWasUpdatedFromStorage) {
                ...
                if (storedConsent) {
                    ...
                    // Zde je důležité, že updateConsent nevoláme s druhým parametrem true,
                    // protože toto není první rozhodnutí v session (bylo načteno z LS).
                    this.updateConsent(storedConsent, false); 
                    this.initialConsentDecisionMadeThisSession = true; // Označíme, že rozhodnutí už padlo
                    if (this.elements.banner) this._hideElement(this.elements.banner);
                } else {
